...
function obterPermissoesSelecionadas() {
    const checkboxesMarcados = document.querySelectorAll(".permissao-checkbox:checked");
...
function excluirPerfil(id) {
    ...
    const totalPermissoes = perfilEncontrado.permissoes ? perfilEncontrado.permissoes.length : 0;

    document.getElementById("nomePerfilExclusao").textContent = perfilEncontrado.nome;
    document.getElementById("permissoesPerfilExclusao").textContent = totalPermissoes + " permissões vinculadas";
...
function confirmarExclusaoPerfil() {
    ...
    const perfisAtualizados = perfis.filter(function(perfil) {
        return perfil.id !== perfilExcluindoId;
    });
...
document.getElementById("formPerfil").addEventListener("submit", function(event) {
    ...
    if (perfilEditandoId === null) {
        ...
        const perfisAtualizados = perfis.map(function(perfil) {
